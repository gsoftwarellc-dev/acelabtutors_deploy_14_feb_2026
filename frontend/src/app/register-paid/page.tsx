        ...
        const fetchData = async () => {
            ...
            try {
                const [optionsRes, settingsRes] = await Promise.all([
                    api.get('/registration-form-options?form_type=paid'),
            ...
            <main className="flex-1 py-12 px-4">
                <div className="container mx-auto max-w-2xl">
                    <div className="mb-8 flex justify-start">
                        <Link href="/student-registration" className="text-slate-500 hover:text-slate-900 flex items-center gap-1 text-xs font-medium transition-colors">
                    ...
                    <div className="text-center mb-12 space-y-2">
                        {loadingSettings ? (
                            ...
                            <>
                                {settings.title && <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{settings.title}</h1>}
                                ...
                                <p className="text-slate-500 text-sm pt-2">{settings.helper_text || "Please complete the form below to register your child."}</p>
                            </>
                        )}
                    </div>
                    ...
                    <form className="space-y-12" onSubmit={handleSubmit}>
                        ...
                        <section className="space-y-8">
                            <h2 className="text-sm font-bold text-slate-900 border-b pb-2">Class & Subject Selection *</h2>
                                ...
                                <div className="space-y-12">
                                    ...
                                    {Array.from(new Set(options.map(o => o.category))).map(category => (
                                        <div key={category} className="space-y-6">
                                            <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">{category}</h3>

                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                {options.filter(o => o.category === category).map((option) => (
                                                    <div key={option.id} className="space-y-3">
                                                        <h4 className="text-[11px] font-bold text-slate-800 uppercase tracking-tight">{option.group_name}</h4>
                                                        <div className="space-y-2">
                        ...
                        <section className="space-y-6">
                            ...
                            <div className="space-y-4">
                                <p className="text-[10px] text-slate-500 leading-relaxed">Does your child have any specific needs that our teachers may need information about, placement test data should be found if you please clarify in the box below.</p>
                                <div className="space-y-2">
